...
```
This doesn't seem to be going so well and unsure why. 
Check by doing directly in R to compare.

```{r}
#mm <- rstan::expose_stan_functions('prior_predictive_full.stan')
source('extract_times_and_scaling.R')
times <- extract_times_and_scaling(8,12,7)
N <- 1000
a <- abs(rnorm(N)*100)
print(a)
phi <- abs(0.57+rnorm(N)*(0.118))
y1 <- 15*a*phi*diff(times$ts1[c(1,7)])
hist(y1)
```
