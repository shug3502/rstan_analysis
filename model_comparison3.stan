functions {
  real[] mrnatransport(real t,
    ...
    producers[1] = 0;
    if (theta[3]<0){
      count = 1; //real either first or last part of the B matrix to give each model
    } else {
      count = 256;
    }
    B = to_matrix(x_r[count:(count+255)],16,16);
...
generated quantities {
  real y_pred[T,16];
  real y_ode[T,16];
  y_ode = integrate_ode_rk45(mrnatransport, y0, t0, ts, theta, bothB, x_i );
  for (t in 1:T){
    for (j in 1:16){
      if (j>1){
