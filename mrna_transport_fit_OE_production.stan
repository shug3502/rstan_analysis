...
//need to return transpose
  ...
  real[] mrnatransport(real t,
    ...
    vector[16] dydt;
    matrix[16,16] B;
    vector[16] producers = to_vector(x_r);
    for (i in 1:16) {
      if (producers[i]<0) producers[i] = theta[4];
    }
...
parameters {
  ...
  real<lower=0> alpha; //how many more times productive are active nurse cells in the OE 
}
...
model {
  ...
  for (t in 1:T3){
    z_OE = integrate_ode_rk45(mrnatransport, y0, 0, ts3, theta_OE, to_array_1d(OE_producers[t,]), x_i ); //use overexpression producers
    for (j in 1:16){
      if (j>1){
        y_OE[t,j] ~ neg_binomial_2(z_OE[t,j], sigma);
      } else {
        y_OE[t,j] ~ neg_binomial_2(phi*z_OE[t,j], sigma);    
      }
    }
  }
}
