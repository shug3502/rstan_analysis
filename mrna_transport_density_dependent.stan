...
generated quantities {
  ...
  for (t in 1:T3){
    y_ode_OE = integrate_ode_rk45(mrnatransport_density_dependent, y0, 0, ts3, theta_OE, to_array_1d(OE_producers[t,]), x_i ); //use overexpression producers
    for (j in 1:16){
      if (j>1){
  ...
  */
  ...
  for (t in 1:T3){
    y_lik_ode = integrate_ode_rk45(mrnatransport_density_dependent, y0, 0, ts3, theta_OE, to_array_1d(OE_producers[t,]), x_i );
    for (j in 1:16){
      if (j>1) {
