...
overexpression_inference <- function(identifier='v206',use_real_data=TRUE,run_mcmc=TRUE,nSamples=15,nTest=3,
                                             parametersToPlot = c("theta","phi","sigma"),verbose=TRUE,compare_via_loo=FALSE,
                                             show_diagnostic_plots=TRUE, test_on_mutant_data=TRUE){
  library(rstan)
  library(mvtnorm)
  library(dplyr)
  ...
  if (use_real_data){
    ...
    if (test_on_mutant_data){
      ...
      test_data = rbind(data[seq_len(nSamples),],overexpression_data)
  ...
  ##########################
  if (run_mcmc) {
    estimates <- stan(file = 'mrna_transport_overexpression.stan',
                      data = list (
                        y = exp_data,
                        T1  = nSamples,
                        T2 = nSamples+nTest,
