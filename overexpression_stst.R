...
overexpression_stst <- function(identifier='MCv201',use_real_data=TRUE,run_mcmc=TRUE,nSamples=1,nTest=0,
                                      parametersToPlot = c('nu','xi','phi'),verbose=TRUE,
  ...
  if (use_real_data){
    ...
    data = matrix(as.numeric(read.csv('data/exp_data_overexpression.csv',sep=',',header=FALSE,stringsAsFactors = FALSE)),ncol=16,byrow=TRUE)
    ...
    normalised_data = data %>% my_normaliser #divide by amount in oocyte to normalise for stst
#    normalised_data = data/data[1,1]
