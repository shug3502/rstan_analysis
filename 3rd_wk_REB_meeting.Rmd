...
analyse_given_model <- function(nTestOE,mID='M0'){
  #mID is a string containing the model ID
  library(loo)
  log_lik_matrix = matrix(full_log_lik %>% filter(modelID==mID) %>% .$ll,ncol=nTestOE,byrow=TRUE)
  loo1 <- loo(log_lik_matrix)
 return(loo1) 
}

aM_list <- purrr::map(models_to_sim, function(x) analyse_given_model(nTestOE,x))
