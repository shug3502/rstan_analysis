...
run_model_comparison_stst_with_decay <- function(identifier='MC_decay_v100',use_real_data=TRUE,run_mcmc=FALSE,nSamples=15,nTest=5,
                                      parametersToPlot = c('nu','xi','phi','gamma'),verbose=FALSE,compare_via_loo=TRUE,
  ...
  if (use_real_data){
    ...
    mc <- stan_model('model_comparison_at_stst_with_decay.stan')
  ...
  ##########################
  if (run_mcmc) {
    estimates <- stan(file = 'model_comparison_at_stst_with_decay.stan',
                      data = list (
                        T1 = nSamples,
                        T2 = nTest,
